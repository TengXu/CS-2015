...
public class MiniGoogle {
  ...
  private static void titleSearch(Scanner s, ArticleTable D) {
   ...
   public static String phraseSearch(String s, ArticleTable T){
     T.reset();
     while(T.hasNext()) {
       Article a = T.next(); 
       double result = getCosineSimilarity(s, a.body);
       a.putCS(result);
       if(result > 0.001){
         H.insert(a);        
       }     
     ...
     if((first = H.getMax()) == null)
       x += "No matching articles found!";
     else{
       x += "Match 2 with cosine similarity of " + first.cosineSimilarity+"\n"+"\n";
     x += first;
     if((first = H.getMax()) != null){
       x += "Match 2 with cosine similarity of " + first.cosineSimilarity+"\n"+"\n";
        x += first;
     ...
     if((first = H.getMax()) != null){
        x += "Match 3 with cosine similarity of " + first.cosineSimilarity +"\n"+"\n";
        x += first;
     }
     }
     return(x);
    ...
    do {
      ...
      switch (choice) {
        ...
        String k = user.nextLine();
          System.out.println(phraseSearch(k, L));
          break;          
